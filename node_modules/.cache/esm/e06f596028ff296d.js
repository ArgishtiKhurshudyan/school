let Teacher,Learner,Topic,Gender,Image,Joi,fs,stringify,multerMiddleware;_28f‍.x([["createTeacher",()=>createTeacher],["getTeacher",()=>getTeacher],["updateTeacher",()=>updateTeacher],["deleteTeacher",()=>deleteTeacher],["exportTeachers",()=>exportTeachers]]);_28f‍.w("../models",[["Teacher",["Teacher"],function(v){Teacher=v}],["Learner",["Learner"],function(v){Learner=v}],["Topic",["Topic"],function(v){Topic=v}],["Gender",["Gender"],function(v){Gender=v}],["Image",["Image"],function(v){Image=v}]]);_28f‍.w("joi",[["default",["Joi"],function(v){Joi=v}]]);_28f‍.w("fs",[["default",["fs"],function(v){fs=v}]]);_28f‍.w("csv-stringify",[["stringify",["stringify"],function(v){stringify=v}]]);_28f‍.w("../multerMiddleware",[["multerMiddleware",["multerMiddleware"],function(v){multerMiddleware=v}]]);
...
const teacherValidation = Joi.object({
  ...
  try {
   await multerMiddleware(req, res)
    _28f‍.g.console.log("req file", req.files);
    ...
    //   ],
    // })
    // const imgName = req.files.map((i) => i.originalname)
    // console.log(imgName[0])
    // stringify({
    //   header: true
    // }, function (err, output) {
    //   fs.writeFile(`./resources/images/${imgName[0]}`, req.files, {},function (err){});
    // })
    return res.status(200).json({ teacher: 'teacher created success!', data: 'teacherFind' })
  } catch (err) {
    _28f‍.g.console.log("error", err)
